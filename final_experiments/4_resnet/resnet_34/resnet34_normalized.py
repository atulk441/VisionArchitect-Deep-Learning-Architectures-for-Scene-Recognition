...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes, use_batchnorm=True):
        super().__init__()
        self.in_planes = 64
        self.use_batchnorm = use_batchnorm
    ...
    def _make_layer(self, block, planes, num_blocks, stride):
        ...
        for _ in range(1, num_blocks):
            layers.append(block(self.in_planes, planes, stride=1, use_batchnorm=self.use_batchnorm))
...
if __name__ == '__main__':
    ...
    dir_name = f"norm_results_resnet_dr_{param_grid['dropout'][0]}"
    ...
    for setting in product(*param_grid.values()):
        ...
        model = ResNet(BasicBlock, layers=[3, 4, 6, 3], num_classes=num_classes,
