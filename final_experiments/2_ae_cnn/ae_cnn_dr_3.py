...
class Decoder(nn.Module):
    def __init__(self, use_batchnorm=True, dropout=0.3, pooling='max'):
        ...
        for i in range(len(channels) - 1):
            ...
            if dropout > 0 and channels[i + 1] != 3 and i in dropout_layers:
                layers.append(nn.Dropout(dropout))
...
if __name__ == '__main__':
    ...
    dir_name = f"results_dr_{param_grid['dropout'][0]}"
    ...
    for setting in product(*param_grid.values()):
        ...
        with torch.no_grad():
            encoded_features = encoder(sample_batch)
        encoded_channels = encoded_features.shape[1]
        encoded_spatial_dim = encoded_features.shape[2]
