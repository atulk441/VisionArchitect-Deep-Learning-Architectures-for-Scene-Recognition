...
class SimpleCNNWithFPN(nn.Module):
    ...
    def forward(self, x):
        ...
        smoothed_feats = [s_conv(f) for f, s_conv in zip(lateral_feats, self.smooth_convs)]

        pooled_feats = [self.global_pool(f).flatten(1) for f in smoothed_feats]
...
if __name__ == '__main__':
    ...
    dir_name = f"basic_cnn_results_{number_blocks}"
    ...
    for setting in product(*param_grid.values()):
        ...
        model = SimpleCNNWithFPN(num_classes,
        ...
        if best_val > global_best_val:
            ...
            print(f"New global best from run {run_id} with val_acc={best_val:.4f}")
