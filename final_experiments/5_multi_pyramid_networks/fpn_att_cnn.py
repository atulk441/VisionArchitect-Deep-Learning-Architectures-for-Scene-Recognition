...
class SimpleCNN(nn.Module):
    def __init__(self, num_classes, filters_list, dropout=0.3, use_batchnorm=False, pooling='max', fpn_channels=256):
        super().__init__()
        ...
        for i, out_channels in enumerate(filters_list):
            ...
            if i in dropout_layers and dropout > 0:
                block.append(nn.Dropout(dropout))

            self.feature_blocks.append(nn.Sequential(*block))
            self.intermediate_channels.append(out_channels)
            in_channels = out_channels

        self.out_channels = fpn_channels
        self.lateral_convs = nn.ModuleList([
            nn.Conv2d(c, self.out_channels, kernel_size=1) for c in self.intermediate_channels
        ])
        self.smooth_convs = nn.ModuleList([
            nn.Conv2d(self.out_channels, self.out_channels, kernel_size=3, padding=1)
            for _ in self.intermediate_channels
        ])

        self.global_pool = nn.AdaptiveAvgPool2d((1,1))
        self.classifier = nn.Linear(self.out_channels * len(filters_list), num_classes)
    ...
    def forward(self, x):
        ...
        for block in self.feature_blocks:
            x = block(x)
            feats.append(x)
        lateral_feats = [l_conv(f) for f, l_conv in zip(feats, self.lateral_convs)]

        for i in range(len(lateral_feats)-1, 0, -1):
            up = F.interpolate(lateral_feats[i], size=lateral_feats[i-1].shape[-2:], mode='nearest')
            lateral_feats[i-1] = lateral_feats[i-1] + up

        smoothed = [s_conv(f) for f, s_conv in zip(lateral_feats, self.smooth_convs)]
        pooled = [self.global_pool(f).flatten(1) for f in smoothed]
